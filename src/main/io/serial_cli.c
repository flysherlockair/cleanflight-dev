...
static void cliSetVar(const configValue_t *var, const int_float_value_t value);
static void cliPrintVar(const configValue_t *var, uint32_t full);
...
{
    uint32_t i;
    const configValue_t *value;
    for (i = 0; i < configVarDefCount; i++) {
        value = &configVarDefs[i];

        if ((value->type & mask) == 0) {
            continue;
        }

        printf("set %s = ", configVarDefs[i].name);
        cliPrintVar(value, 0);
        cliPrint("\r\n");
    }
}
...
static void cliSet(char *cmdline)
{
    ...
    if (len == 0 || (len == 1 && cmdline[0] == '*')) {
        ...
        for (i = 0; i < configVarDefCount; i++) {
            ...
            if (strncasecmp(cmdline, configVarDefs[i].name, strlen(configVarDefs[i].name)) == 0 && variableNameLength == strlen(configVarDefs[i].name)) {
                if (valuef >= configVarDefs[i].min && valuef <= configVarDefs[i].max) { // here we compare the float value since... it should work, RIGHT?
                    int_float_value_t tmp;
                    if (configVarDefs[i].type & VAR_FLOAT)
                        tmp.float_value = valuef;
                    else
                        tmp.int_value = value;
                    cliSetVar(val, tmp);
                    printf("%s set to ", configVarDefs[i].name);
                    cliPrintVar(val, 0);
                } else {
...
static void cliGet(char *cmdline)
{
    ...
    for (i = 0; i < configVarDefCount; i++) {
        if (strstr(configVarDefs[i].name, cmdline)) {
            val = &configVarDefs[i];
            printf("%s = ", configVarDefs[i].name);
            cliPrintVar(val, 0);
            cliPrint("\r\n");
